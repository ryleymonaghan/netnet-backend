...
function classify(tx, accountKind) {
  ...
  if (shares) {
    const acct = SHARES_SUFFIX_TO_ACCOUNT[shares[2]];
    // A SHARES suffix that isn't one of our 5 tracked accounts' legacy codes
    // isn't a parse failure - it's a transfer to/from another member's
    // account (SC Federal is a credit union; "shares" spans membership, not
    // just our four business accounts). Not internal, so it never nets to
    // zero here - it needs a human to say what it actually is.
    if (!acct) return { kind: 'external', counterpartyAccount: null };
    return { kind: 'checkg', counterpartyAccount: acct };
  }
...
//
// Tags the underlying transaction objects in place (tx.transferTag) so the
// category engine can recognize them without re-deriving the classification.
function matchTransfers(sections) {
  ...
  for (const sec of sections) {
    for (const tx of sec.transactions) {
      ...
      if (info.kind === 'external') {
        tx.transferTag = direction === 'in' ? 'EXTERNAL_INBOUND' : 'EXTERNAL_OUTBOUND';
  ...
  for (const out of outbound) {
    ...
    if (best) {
      best.matched = true;
      out.matched = true;
      out.tx.transferTag = 'INTERNAL_TRANSFER';
      best.tx.transferTag = 'INTERNAL_TRANSFER';
  ...
  return { matches, unmatched, external, volumeEliminated };
}
